<template>
  <div>
        ...
        <div class="w-100">
          <div class="input-group mb-2">
            ...
            <input
              type="text"
              class="form-control"
              :placeholder="$t('tabs.paymentsTab.search.placeholder')"
            />
          </div>
        </div>
      </div>
            ...
            <thead>
              <tr>
                ...
                <th class="position-sticky" style="top: 0">
                  {{ $t('tabs.paymentsTab.content.table.fields.status') }}
                </th>
                <th class="position-sticky" style="top: 0">
                  {{ $t('tabs.paymentsTab.content.table.fields.amount') }}
                </th>
                <th class="position-sticky" style="top: 0">
                  {{ $t('tabs.paymentsTab.content.table.fields.dueDate') }}
                </th>
                <th class="position-sticky" style="top: 0">
                  {{ $t('tabs.paymentsTab.content.table.fields.markPaid') }}
                </th>
    ...
    <DeleteConfirmModal
      :id="`delete_payments_confirm_modal`"
      @confirm="deleteItem"
    />
  </div>
</template>
...
export default {
  ...
  methods: {
    updateModal(type, id) {
      ...
      this.$bvModal.show(`projects_edit_payments_modal`)
    },
    deleteConfirm(id) {
      this.current.id = id
      this.$bvModal.show(`delete_payments_confirm_modal`)
    },
    deleteItem() {
      this.$axios
        .delete(`${process.env.API_DOMAIN}/api/v_1/note/${this.current.id}`)
        .then((res) => {
          this.$bvModal.hide(`delete_payments_confirm_modal`)
