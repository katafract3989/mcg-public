<template>
  <div>
        ...
        <div class="w-100">
          <div class="input-group mb-2">
            ...
            <input
              type="text"
              class="form-control"
              :placeholder="$t('tabs.notesTab.search.placeholder')"
            />
          </div>
        </div>
      </div>
            ...
            <thead>
              <tr>
                ...
                <th class="position-sticky" style="top: 0">
                  {{ $t('tabs.notesTab.content.table.fields.description') }}
                </th>
                <th class="position-sticky" style="width: 250px; top: 0">
                  {{ $t('tabs.notesTab.content.table.fields.date') }}
                </th>
                <th class="position-sticky" style="width: 32px; top: 0"></th>
              </tr>
            </thead>
    ...
    <DeleteConfirmModal id="delete_note_confirm_modal" @confirm="deleteItem" />
  </div>
</template>
...
export default {
  ...
  methods: {
    deleteConfirm(id) {
      this.current.id = id
      this.$bvModal.show('delete_note_confirm_modal')
    },
    deleteItem() {
