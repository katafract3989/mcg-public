        ...
        <div class="w-100">
          <div class="input-group mb-2">
            ...
            <input
              type="text"
              class="form-control"
              :placeholder="$t('tabs.tasksTab.search.placeholder')"
            />
          </div>
        </div>
      </div>
          ...
          >
            <thead>
              <tr>
                ...
                <th class="position-sticky" style="width: 250px; top: 0">
                  {{ $t('tabs.tasksTab.content.table.fields.created') }}
                </th>
                <th class="position-sticky" style="width: 250px; top: 0">
                  {{ $t('tabs.tasksTab.content.table.fields.createdBy') }}
                </th>
                <th class="position-sticky" style="top: 0">
                  {{ $t('tabs.tasksTab.content.table.fields.name') }}
                </th>
                <th class="position-sticky" style="top: 0">
                  {{ $t('tabs.tasksTab.content.table.fields.status') }}
                </th>
                <th class="position-sticky" style="top: 0">
                  {{ $t('tabs.tasksTab.content.table.fields.dueDate') }}
                </th>
                <th class="position-sticky" style="width: 32px; top: 0"></th>
              </tr>
            </thead>
            <tbody>
              <template v-if="Object.keys(content).length > 0">
                <tr v-for="(item, i) in content" :key="`lead-task-${i}`">
                  ...
                  <td class="vertical-center">{{ item.name }}</td>
                  <td class="vertical-center">-</td>
                  <td class="vertical-center">
                    {{ $moment(item.expired_at).format('MM/DD/YYYY hh:mm A') }}
                  </td>
                  <td class="vertical-center">
                    <a
...
export default {
  ...
  methods: {
    ...
    deleteItem() {
      ...
      // this.$axios
      //   .delete(`${process.env.API_DOMAIN}/api/v_1/tasks/${this.current.id}`)
      //   .then((res) => {
