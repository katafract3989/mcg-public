<template>
  <div class="row mt-4">
            ...
            <div>
              <div class="input-group mb-2">
                ...
                <input
                  ...
                  class="form-control"
                  :placeholder="
                    $t(`pages.permissionsPage.list.search.placeholder`)
                  "
                />
              </div>
            </div>
                  ...
                  <thead>
                    <tr>
                      ...
                      <th class="text-primary" style="width: 35%">
                        {{ $t(`pages.permissionsPage.list.fields.name`) }}
                      </th>
                      <th class="text-primary" style="width: 58%">
                        {{
                          $t(`pages.permissionsPage.list.fields.description`)
                        }}
                      </th>
                      ...
                      <tr>
                        <td colspan="6">
                          <a
                            ...
                            @click="
                              $bvModal.show(
                                'permissions_create_permission_modal'
                              )
                            "
                          >
                            {{ $t('pages.permissionsPage.list.empty') }} +
                          </a>
                        </td>
                      </tr>
                    </template>
    ...
    <DeleteConfirmModal
      :id="`delete_permission_confirm_modal`"
      @confirm="deleteItem"
    >
      <template #notification>
        <div class="text-danger">
          Make sure you don't use this permission first
        </div>
      </template>
    </DeleteConfirmModal>
  </div>
...
export default {
  ...
  methods: {
    ...
    deleteConfirm(id) {
      this.del = id
      this.$bvModal.show(`delete_permission_confirm_modal`)
    },
    deleteItem() {
      this.$bvModal.hide(`delete_permission_confirm_modal`)
